...
for (output.var in c('both')){
  ...
  if (output.var == 'inflam'){
    ...
    both.data = rbind(data, otherdata)
    # both.data %>% pivot_wider(id_cols = SampleName, values_from = NPQ, names_from = Target)
    both.data = both.data %>% pivot_wider(id_cols = SampleName, values_from = NPQ, names_from = Target) %>%
      column_to_rownames('SampleName')
    both.data
    normalized_data <- limma::normalizeBetweenArrays(both.data %>% t(), method = "quantile")
    normalized_data = normalized_data %>% t() %>% data.frame() %>% select(-DDC)
    normalized.long = normalized_data %>% data.frame() %>% rownames_to_column('SampleName') %>%
      pivot_longer(cols = !SampleName, names_to = 'Target', values_to = 'NPQ')

    data
    data = data %>% select(SampleName) %>% distinct() %>% left_join(normalized.long) %>%
  ...
  for (test.fold.i in 1:5){
    ...
    for (model.type in model.list){
      ...
      basel.pred.folder = paste0('figures/prediction/basel.intensity.normed/')
