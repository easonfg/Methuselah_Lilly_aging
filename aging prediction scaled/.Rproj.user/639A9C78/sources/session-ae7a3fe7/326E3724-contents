    ...
    for (model.type in model.list){
      ...
      if (model.type == 'elastic.net'){
        if (test.fold.i < 6) {
          ...
          svm1 = read_csv(paste0('figures/model.iter.12/elastic.net/both/aggregated.performance.rank.csv'))
          sorted.res = svm1
          j = 1
        }
        glm.manual = glmnet(
      ...
      if (model.type == 'SVM'){
        if (test.fold.i < 6) {
          ...
          svm1 = read_csv(paste0('figures/model.iter.12/SVM/both/aggregated.performance.rank.csv'))
          sorted.res = svm1
          j = 1
        }
        m_svm_untuned <- svm(
      ...
      if (model.type == 'lightGBM'){
        if (test.fold.i < 6) {
          ...
          svm1 = read_csv(paste0('figures/model.iter.12/lightGBM//both/aggregated.performance.rank.csv'))
          sorted.res = svm1
          j = 1
        }
        # browser()
      ...
      basel.pred.folder = paste0('figures/prediction/basel.whole.training/')
