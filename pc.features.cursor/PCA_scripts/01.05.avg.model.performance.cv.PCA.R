### average the model performance across the 5 fold cv to pick the best hyperparameter for PCA models

# detach(package:plyr)
# library(dplyr)
library(tidyverse)

# model.iiiii = 'elastic.net'
# model.i = 'lightGBM'
for (model.i in c('elastic.net.PCA', 'SVM.PCA', 'lightgbm.PCA')){
  model.i

  if (model.i == 'elastic.net.PCA'){
    iter.i = 'a'
  } else if (model.i == 'SVM.PCA') {
    iter.i = 'j'
  } else if (model.i == 'lightgbm.PCA') {
    iter.i = 'a'
  }

  model.performance.folder = paste0('PCA_results/model.iter.12/', model.i, '/both/')
...
cat("PCA model hyperparameter aggregation completed!\n")
