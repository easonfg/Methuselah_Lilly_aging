...
for (output.var in c('both')){
  ...
  save(pca_result, n_pcs, variance_explained, file = paste0('PCA_results/PCA_results_Basel_prediction_', output.var, '.RData'))
  ...
  # for (test.fold.i in 1:5){
    ...
    ################################################################################
    ### APPLY PCA TRANSFORMATION TO BASEL DATA ###

    # Load the PCA results from Methuselah data to get the transformation matrix
    load(paste0('PCA_results/PCA_results_Basel_prediction_', output.var, '.RData'))
    ...
    # Get the protein columns from Basel data (excluding Sex)
    protein_cols_basel <- colnames(otherdata.wide)[!colnames(otherdata.wide) %in% c("Sex")]
    cat("Number of proteins in Basel data:", length(protein_cols_basel), "\n")
    ...
    protein_cols_basel_clean <- gsub("[^[:alnum:]]", "", protein_cols_basel)
    ...
    basel_clean_to_original <- setNames(protein_cols_basel, protein_cols_basel_clean)
    
    # Find common proteins using cleaned names
    common_proteins_clean <- intersect(methuselah_proteins_clean, protein_cols_basel_clean)
    ...
    common_proteins_basel <- basel_clean_to_original[common_proteins_clean]
    ...
    if(length(common_proteins_methuselah) > 0) {
      ...
      # Extract the common proteins from Basel data using Basel protein names
      basel_protein_data <- otherdata.wide[, common_proteins_basel, drop = FALSE]
      ...
      # IMPORTANT: We need to center and scale the Basel data using the SAME parameters as Methuselah
      ...
      # Note: prcomp() with scale.=TRUE and center=TRUE stores these in the original protein data that was used for PCA
      ...
      # First, we need to ensure the Basel data has the same structure as the original
      # Create a temporary dataset with the same proteins as Methuselah (fill missing with 0)
      basel_proteins_complete <- matrix(0, nrow = nrow(basel_protein_data), ncol = length(methuselah_proteins))
      colnames(basel_proteins_complete) <- methuselah_proteins
      
      # Fill in the common proteins
      basel_proteins_complete[, common_proteins_methuselah] <- as.matrix(basel_protein_data)

      # Now apply the PCA transformation using predict()
      basel_pc_scores <- predict(pca_result, newdata = basel_proteins_complete)

      # Take only the first n_pcs components
      basel_pc_scores <- basel_pc_scores[, 1:n_pcs, drop = FALSE]
      colnames(basel_pc_scores) <- paste0("PC", 1:n_pcs)

      # Create new Basel dataset with PCs
      basel_pc_data <- data.frame(
      ...
      otherdata.wide <- as.matrix(basel_pc_data[, -1]) # Remove SampleName, keep PCs and Sex
      rownames(otherdata.wide) <- basel_pc_data$SampleName

      cat("Successfully transformed Basel data to", n_pcs, "PCs using proper PCA transformation\n")
    ...
    # vali <- vali %>% data.frame() %>% select(all_of(common.proteins)) %>%
    #   scale(., center = scale.means, scale = scale.sds)
    ...
    for (model.type in model.list){
      ...
      basel.pred.folder = paste0('PCA_results/prediction/basel.whole.training.PCA/')
...
cat("PCA-based Basel prediction completed!\n")
